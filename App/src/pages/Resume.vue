        ...
        <main class="pt-16">
            <section id="resume" class="container mx-auto p-8">
                <div class="flex items-center justify-center space-x-4">
                    <h1 class="text-4xl font-bold mb-6 text-center text-black">My Resume</h1>
                    <a @click="downloadResume()"  download class="mb-6 cursor-pointer" title="Download Resume">
                        <i class="fa-solid fa-file-arrow-down text-black text-4xl"></i>
                    </a>
                </div>
                ...
                <div class="grid sm:grid-cols-2 grid-cols-1 gap-6">
                    <div class="bg-white p-6 rounded-lg shadow-md">
                        <h2 class="text-xl  font-semibold mb-4 text-gray-600">Education</h2>
                        <ul class="space-y-3">
                            <li v-for="edu in userInfo.Educations" :key="edu.id" class="border-b-2 pb-2 text-black">
                    ...
                    <div class="bg-white p-6 rounded-lg shadow-md">
                        <h2 class="text-xl text-gray-600 font-semibold mb-4">Skills</h2>
                ...
                <div class="grid sm:grid-cols-2 grid-cols-1 gap-6 mt-6">
                    <div class="bg-white p-6 rounded-lg shadow-md">
                        <h2 class="text-xl text-gray-600 font-semibold mb-4">Projects</h2>
                        <ul v-if="projects.length" class="space-y-3">
                            <li v-for="project in projects" :key="project.id" class="border-b-2 pb-2">
                                <p class="font-medium text-black">{{ project.name }}</p>
                    ...
                    <div class="bg-white p-6 rounded-lg shadow-md">
                        <h2 class="text-xl text-gray-600 font-semibold mb-4">Experience</h2>
                        <ul v-if="experiences.length" class="space-y-3">
                            <li v-for="exp in experiences" :key="exp.id" class="border-b-2 pb-2">
                                <p class="font-medium text-black">{{ exp.title }}</p>
                ...
                <div v-if="userInfo.Certifications.length" class="bg-white p-6 rounded-lg shadow-md mt-6">
                    <h2 class="text-xl text-gray-600 font-semibold mb-4">Certifications</h2>
                    <ul class="space-y-3">
                        <li v-for="cert in userInfo.Certifications" :key="cert.id" class="border-b-2 pb-2">
                            <p class="font-medium text-black">{{ cert.title }} - {{ cert.issuer }}</p>
                ...
                <!-- Diplomas -->
                <div v-if="userInfo.Diplomas.length" class="bg-white p-6 rounded-lg shadow-md mt-6">
                    <h2 class="text-xl text-gray-600 font-semibold mb-4">Diplomas</h2>
                    <ul class="space-y-3">
                        <li v-for="dipl in userInfo.Diplomas" :key="dipl.id" class="border-b-2 pb-2">
                            <p class="font-medium text-black">{{ dipl.title }} - {{ dipl.issuer }}</p>
                            <p class="text-sm text-gray-500">Completed in {{ dipl.date }}</p>
                        </li>
                    </ul>
                </div>
                ...
                <div v-if="userInfo.Languages.length" class="bg-white p-6 rounded-lg shadow-md mt-6">
                    <h2 class="text-xl text-gray-600 font-semibold mb-4">Languages</h2>
                    <ul class="space-y-3">
                        <li v-for="lang in userInfo.Languages" :key="lang.name" class="border-b-2 pb-2">
                            <p class="font-medium text-black">{{ lang.name }}</p>
                ...
                <div v-if="userInfo.AdditionalSkills.length" class="bg-white p-6 rounded-lg shadow-md mt-6">
                    <h2 class="text-xl text-gray-600 font-semibold mb-4">Additional Skills</h2>
                    <ul class="space-y-3">
                        <li v-for="skill in userInfo.AdditionalSkills" :key="skill" class="border-b-2 pb-2">
                            <p class="font-medium text-black">{{ skill }}</p>
                        </li>
                    </ul>
                </div>
            </section>
